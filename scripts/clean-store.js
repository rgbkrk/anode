...
async function main() {
  log('\n🧹 Cleaning all LiveStore storage and build artifacts...', colors.bright + colors.cyan);

  let totalRemoved = 0;

  // 1. Cloudflare Worker storage (.wrangler directories)
  log('\n📦 Cloudflare Worker Storage:', colors.bright + colors.blue);
  if (await removeDirectory(join(rootDir, 'packages/docworker/.wrangler'), 'Cloudflare .wrangler directory')) {
    totalRemoved++;
  }

  // 2. Kernel server storage (tmp directories)
  log('\n🐍 Kernel Server Storage:', colors.bright + colors.blue);
  if (await removeDirectory(join(rootDir, 'packages/dev-server-kernel-ls-client/tmp'), 'Kernel server tmp directory')) {
    totalRemoved++;
  }
  ...
  for (const pkg of packages) {
    if (await removeDirectory(join(rootDir, `packages/${pkg}/dist`), `${pkg} dist directory`)) {
      totalRemoved++;
    }
  }
  ...
  if (await removeDirectory(join(rootDir, 'packages/web-client/node_modules/.vite'), 'Vite cache directory')) {
    totalRemoved++;
  }
  ...
  const cachePatterns = [
    ...
    { path: join(rootDir, 'packages/web-client'), pattern: /^\.next$/, desc: 'Next.js cache' },
    { path: join(rootDir, 'packages/web-client'), pattern: /^build$/, desc: 'Web client build' },
    { path: join(rootDir, 'packages/web-client'), pattern: /^out$/, desc: 'Web client output' },
  ];
  ...
  for (const dir of dbDirs) {
    const removed = await removePattern(dir, /\.(db|sqlite|sqlite3|log)$/, 'database files');
    if (removed > 0) totalRemoved += removed;
  }
  ...
  if (totalRemoved > 0) {
    log(`✅ Cleanup complete! Removed ${totalRemoved} items.`, colors.bright + colors.green);
    ...
    log('\n🎯 You now have a completely clean state for development!', colors.green);
  } else {
